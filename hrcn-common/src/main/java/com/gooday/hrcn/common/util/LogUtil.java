...
public class LogUtil {
    ...
    private static final Logger loggerService = LoggerFactory.getLogger("SERVICE");
    private static final Logger loggerAction = LoggerFactory.getLogger("ACTION");
    ...
    private static String getCauseMsg(Exception e) {
        ...
        String stacks = Arrays.asList(e.getStackTrace()).stream().filter(el -> el.getClassName().contains(PACKAGE_FLAG)).filter(el -> !el.getClassName().contains("hrcn")).filter(el -> el.getLineNumber() != -1).map(el -> {
            StringBuffer buf = new StringBuffer();
            buf.append(el.getClassName()).append(".").append(el.getMethodName()).append("(").append(el.getLineNumber()).append(")");
            return buf.toString();
        }).collect(Collectors.joining(" \r\nat "));
        if (ValidUtil.isEmpty(stacks)) {
            stackBuf.append(" \r\nat ");
